  ...
  configure do
    ...
    if ENV.fetch('ENABLE_SAML_AUTH', false) == 'true'
      ...
      if ENV.fetch('METADATA_ENDPOINT', nil)
        ...
        idp_metadata = idp_metadata_parser.parse_remote_to_hash(ENV.fetch('METADATA_ENDPOINT', nil))

        omniauth_provider :saml, idp_metadata.merge(
          assertion_consumer_service_url: ENV.fetch('ACS_URL', nil),
        ...
        omniauth_provider :saml, {
          assertion_consumer_service_url: ENV.fetch('ACS_URL', nil),
          sp_entity_id: ENV.fetch('SP_ENTITY_ID', nil),
          idp_sso_service_url: ENV.fetch('IDP_SERVICE_URL', nil),
